...
module.exports.run = async (bot, message, args) => {
    ...
    if(!message.member.roles.has(staff.id)) {
        return message.channel.send('You are not an ``Administrator``.');
         ...
         banMember.ban(1)
         setTimeout(() => {
            message.guild.unban(members.id);
        }, 0);
             message.channel.send(members.user.username + " was Softbanned from Strand's public discord!")
             console.log("[StrandBot Log] " + members.user.username + ' was Softbanned from Strands Public Discord')
